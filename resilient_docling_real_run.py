"""Real Docling run — the EMPIRICAL test of the decreasing-backoff hypothesis.
    ...
    uv run python resilient_docling_real_run.py                 # dry run: lists what it WOULD do
    uv run python resilient_docling_real_run.py --go            # actually runs Docling
    uv run python resilient_docling_real_run.py --go --schedule 20,10,5,2,1 --limit 3
...
def main() -> int:
    ...
    from ocr_bifunction.docling_page_range_converter import (
        make_docling_page_range_converter,
    )
    from ocr_bifunction.resilient_conversion import reconcile_page_range_conversion
