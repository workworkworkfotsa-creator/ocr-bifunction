"""Suggestion lane runner — deterministic-first, SLM last resort, anchors re-verified (BRIEF del. 2).
    ...
    uv run python suggestion_check.py <doc> [--category facture]     # match wins -> SLM asleep
    uv run python suggestion_check.py <doc> --force-slm               # skip the match, prove the SLM
    uv run python suggestion_check.py <doc> --stage --store PATH      # stage a verified suggestion in D3
...
from ocr_bifunction.template import load_templates, match_template
