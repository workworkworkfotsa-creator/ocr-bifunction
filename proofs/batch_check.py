"""Batch orchestrator runner — the end-to-end BATCH backbone on a lot of real documents.
    ...
    uv run python proofs/batch_check.py <doc> [<doc> ...]           # each file = one single doc
    uv run python proofs/batch_check.py --ci recto.png verso.png    # group files as ONE CI upload
    uv run python proofs/batch_check.py --escalate <doc> ...        # allow LightOCR (needs llama-swap)
...
from ocr_bifunction.flow.orchestrator import (
    CI_CATEGORY,
    BatchItem,
    BatchResult,
    DocumentRecord,
...
from ocr_bifunction.extraction.template import field_payload, load_templates
...
def main() -> int:
    ...
    if arguments.suggest:
        from ocr_bifunction.extraction.suggestion import (
            SuggestionOutcome,
            suggest_template,
        )
