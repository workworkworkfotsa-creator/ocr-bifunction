"""Reconcile a recto (template) against a verso MRZ — end-to-end on real cards.
    ...
    uv run python proofs/reconcile_check.py <recto_image> <verso_image>
...
from ocr_bifunction.extraction.mrz import MrzFields, extract_mrz_lines, parse_mrz
from ocr_bifunction.reading.preprocess import EnhancePreprocessor
from ocr_bifunction.reading.reader import OcrEngine, read_document
from ocr_bifunction.extraction.reconcile import reconcile
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "usage: uv run python proofs/reconcile_check.py <recto_image> <verso_image>"
        )
        sys.exit(1)
