"""CI anchor-lane fingerprint — the ISO-OUTPUT oracle for changes to the geometry rules.
    ...
    uv run python proofs/ci_geometry_fingerprint.py                 # default: inputs/recto_verso.pdf
    uv run python proofs/ci_geometry_fingerprint.py <recto_verso>   # any CI upload (1+ files)
...
cannot prove that — only replaying a real document can. Capture before, capture after, `diff`.
    ...
    uv run python proofs/ci_geometry_fingerprint.py > before.txt
    # ... change the geometry ...
    uv run python proofs/ci_geometry_fingerprint.py > after.txt
...
from ocr_bifunction.flow.pipeline import process_ci_submission
