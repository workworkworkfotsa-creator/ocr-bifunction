"""UI + review surface smoke — drive the two pages' endpoints end to end (no llama).
    ...
    uv run python proofs/ui_smoke.py <facture_pdf> <courrier_pdf>
...
from ocr_bifunction.paths import ADAPTERS_DIRECTORY, PROJECT_ROOT, PROOFS_DIRECTORY
...
from ocr_bifunction.storage.review_repository import (  # noqa: E402
    Review,
...
def run(facture_path: Path, courrier_path: Path) -> int:
    ...
    with TestClient(api_maquette.app) as client:
        ...
        draft_output = subprocess.run(
            [
                sys.executable,
                str(PROOFS_DIRECTORY / "draft_check.py"),
