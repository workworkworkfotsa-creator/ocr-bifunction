"""OCR BiFunction — single entry point: one CI recto+verso pair -> record + verdict.
    ...
    uv run python -m ocr_bifunction.adapters.main <recto_image> <verso_image>
...
from ocr_bifunction.flow.pipeline import process_ci_pair
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "usage: uv run python -m ocr_bifunction.adapters.main <recto_image> <verso_image>"
        )
        sys.exit(1)
