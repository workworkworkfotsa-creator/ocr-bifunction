"""Watchdog worker — the SEPARATE process that owns every D1 status transition.
    ...
    uv run python -m ocr_bifunction.adapters.worker_watchdog                 # loop (Ctrl+C to stop after current job)
    uv run python -m ocr_bifunction.adapters.worker_watchdog --once          # one pass and exit (scheduler parity + smokes)
    uv run python -m ocr_bifunction.adapters.worker_watchdog --fake-escalation   # smoke seam: no VLM, no llama
...
from ocr_bifunction.flow.intake import handle_document
from ocr_bifunction.flow.orchestrator import BatchItem
from ocr_bifunction.reading.reader import OcrEngine, TextLine
...
from ocr_bifunction.governance.conformity_policy import SqliteConformityPolicyRepository
from ocr_bifunction.knowledge.context_assembly import collect_validated_attestations
from ocr_bifunction.knowledge.drafting_flow import run_draft_pass
from ocr_bifunction.governance.issuer_registry import SqliteIssuerRegistryRepository
...
def _build_escalation_engine(fake: bool) -> OcrEngine:
    ...
    from ocr_bifunction.reading.engines.lightonocr_engine import LightOnOcrEngine
