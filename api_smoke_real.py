"""API maquette smoke — drive a real CI submission end to end through the HTTP contract.
    ...
    uv run python api_smoke_real.py <recto> <verso>                 # -> validated
    uv run python api_smoke_real.py recto_verso.pdf --expect validated
    uv run python api_smoke_real.py <recto> --expect incomplete     # missing verso
    uv run python api_smoke_real.py <rectoA> <versoB> --expect pending
